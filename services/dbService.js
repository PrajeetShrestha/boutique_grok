...
function seedProducts() {
    db.get('SELECT COUNT(*) as count FROM products', [], (err, row) => {
        ...
        if (row.count === 0) {
            ...
            const stmt = db.prepare(`INSERT INTO products (id, name, category, price, img, color, fabric, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
            ...
            stmt.finalize((err) => {
                ...
                else console.log('Products table seeded with 100 items');
            });
        } else {
